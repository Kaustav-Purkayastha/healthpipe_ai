...
# AI-ENRICHMENT-HOOK (Step 5)
# When Ollama integration is added back, restore these two lines here:
...
class DocumenterAgent:
    ...
    def run(
        ...
        Args:
            ...
            descriptions:       AI-generated column descriptions from Step 5
                                core.enrich.describe_columns() (optional).  When
                                provided, these take precedence over rule-based text.
        ...
        docs = {
            ...
            "data_dictionary": self._generate_data_dictionary(df, descriptions=descriptions),
    ...
    def _generate_data_dictionary(
        ...
        Args:
            df:           The DataFrame to document.
            descriptions: Optional AI-generated descriptions (column → text).  When
                          a column appears in this dict, its description takes
                          precedence over the rule-based fallback.
        ...
        for col in df.columns:
            ...
            # Prefer AI-generated description when available (Step 5 enrichment).
            # AI-ENRICHMENT-HOOK (Step 5) — descriptions dict is populated by
            # core.enrich.describe_columns() and passed in from core.pipeline.
    ...
    def _infer_description(
        ...
        """
        Infer a column description using rule-based pattern matching.

        AI-ENRICHMENT-HOOK (Step 5): In Step 5 this method will first attempt
        an Ollama-generated description and fall back to rules when AI is
        unavailable.  For now, only the rule-based path runs.

        Args:
            column_name:   The column name to describe.
            sample_values: Up to 3 sample values (unused until Step 5 AI hook).

        Returns:
            A human-readable description string.
        """
        # AI-ENRICHMENT-HOOK (Step 5) — insert AI call here before rules:
        #   if _OLLAMA_READY:
        #       ai_desc = self._get_ai_description(column_name, sample_values)
        #       if ai_desc:
        #           return ai_desc
