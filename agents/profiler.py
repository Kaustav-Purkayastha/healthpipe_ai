...
class ProfilerAgent:
    ...
    def detect_pii_columns(self, df: pd.DataFrame) -> list[dict]:
        """
        Detect columns that likely contain PII using deterministic rules only.

        The scan looks at BOTH the column name and its actual values, not the
        name alone — so a geographic column like ``state_name`` (which merely
        *contains* the substring "name") is NOT flagged as personal data.

        Rules, in order, per column:
        0. Geographic guard: if the values are mostly US states, it's geography,
           never PII — skip it.
        1. Strong name hint (email/ssn/phone/npi/address/zip/dob): the column name
           is unambiguous → flag (high).
        2. Value pattern: values match an email/phone/ssn/npi regex → flag (medium).
        3. Weak name hint (name/first/last): ambiguous names, so only flag when the
           VALUES also look like personal names (title-cased text) → flag (medium).

        Returns a list of dicts: {column, reason, confidence}.
        """
        # Unambiguous PII column-name keywords (flag on the name alone).
        STRONG_NAME_HINTS = ["email", "phone", "ssn", "npi", "address", "zip", "dob"]
        # Ambiguous keywords ("state_name", "first_year", "last_updated" are NOT
        # PII) — only flag these when the values corroborate personal names.
        WEAK_NAME_HINTS = ["name", "first", "last"]
        ...
        # US states (abbreviations + full names, incl. DC) — a geographic column,
        # not personal data, even when its column name contains "name".
        _STATE_ABBR = {
        ...
        # A person name: title-cased token(s), e.g. "John", "Mary-Jane Watson".
        _PERSON_NAME = re.compile(r"^[A-Z][a-z]+(?:[ \-'][A-Z][a-z]+)*$")

        def _mostly(sample: list[str], predicate) -> bool:
            vals = [str(v).strip() for v in sample if str(v).strip()]
            if not vals:
                return False
            return sum(1 for v in vals if predicate(v)) / len(vals) >= 0.6
        ...
        for col in df.columns:
            ...
            # 0. Geographic guard — US-state columns are never PII.
            if _mostly(sample, lambda v: v.upper() in _STATE_ABBR or v.lower() in _STATE_NAMES):
                continue

            # 1. Strong (unambiguous) name hint → flag on the name alone.
            strong = next((h for h in STRONG_NAME_HINTS if h in col_lower), None)
            if strong:
                findings.append({"column": col, "reason": f"name_hint:{strong}",
                                 "confidence": "high"})
                continue

            # 2. Value pattern (works regardless of the column name).
            matched_pattern = None
            for pattern_name, pattern in VALUE_PATTERNS:
                hit = sum(1 for v in sample if pattern.search(v))
                if sample and hit / len(sample) > 0.1:
                    matched_pattern = pattern_name
                    break
            if matched_pattern:
                findings.append({"column": col, "reason": f"value_pattern:{matched_pattern}",
                                 "confidence": "medium"})
                continue

            # 3. Weak name hint → only if the VALUES look like personal names.
            weak = next((h for h in WEAK_NAME_HINTS if h in col_lower), None)
            if weak and _mostly(sample, lambda v: bool(_PERSON_NAME.match(v))):
                findings.append({"column": col, "reason": f"name_hint:{weak}",
                                 "confidence": "medium"})

        logger.info("PII scan found %d suspicious columns", len(findings))
        return findings
