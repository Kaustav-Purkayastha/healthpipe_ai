...
from core.config import (
    ...
    OPENFDA_PAGE_SIZE,
    OPENFDA_RATE_LIMIT_SECONDS,
    OPENFDA_MAX_RETRIES,
    OPENFDA_RETRY_BACKOFF_BASE,
)
...
class OpenFDASource(BaseSource):
    ...
    def connect(self) -> bool:
        ...
        """
        try:
            url = f"{OPENFDA_BASE_URL}{OPENFDA_DRUG_EVENT_ENDPOINT}"
    ...
    def extract(
        ...
        while len(all_records) < max_records:
            ...
            # Respect rate limit
            time.sleep(OPENFDA_RATE_LIMIT_SECONDS)
    ...
    def _fetch_page_with_retry(
        ...
        for attempt in range(OPENFDA_MAX_RETRIES):
            try:
                ...
                if response.status_code == 429:
                    ...
                    wait = OPENFDA_RETRY_BACKOFF_BASE * (2 ** attempt)
                    logger.warning(
                        f"Rate limited (429). Waiting {wait}s "
                        f"(attempt {attempt + 1}/{OPENFDA_MAX_RETRIES})"
                    )
                    time.sleep(wait)
                ...
                if attempt < OPENFDA_MAX_RETRIES - 1:
        ...
        logger.error(
            f"OpenFDA request failed after {OPENFDA_MAX_RETRIES} attempts"
        )
        return None
